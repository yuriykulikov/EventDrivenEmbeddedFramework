...
SpiMaster * SpiMaster_init(SPI_t *module, bool lsbFirst, SPI_MODE_t mode, bool clk2x, SPI_PRESCALER_t clockDivision) {
    ...
    module->CTRL = clockDivision | (clk2x ? SPI_CLK2X_bm : 0) | SPI_ENABLE_bm | (lsbFirst ? SPI_DORD_bm : 0)
            | SPI_MASTER_bm | mode; // TODO Add a field to hold the interrupt level which would be used to transmit large packets
...
uint8_t SpiMaster_shiftByte(SpiDevice *device, uint8_t data) {
    ...
    while (!(device->master->module->STATUS & SPI_IF_bm)) {
        nop()
        ;
    }
